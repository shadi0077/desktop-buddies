...
// The game roster: everybody loads, every clip a personality names exists, and
...
//
// The Agent characters have their own test next door (tools/casttest), which
// asks about speech packs, voices and languages — the things these characters
...
for p in Personality.all {
    ...
    for name in ["rest", "arrive", "depart", p.travel.cruise]
    where store.animation(name) == nil { missing.append(name) }
...
check("Max is slower than Skate",
      Personality.maxsor.roaming.speed < Personality.skate.roaming.speed)
check("Earl does not hurry",
      Personality.earltje.roaming.speed < Personality.toejam.roaming.speed
          && Personality.earltje.beatRange.lowerBound > Personality.toejam.beatRange.lowerBound)
